export interface EdgesOptions {
  /** 0-1, lower = keep more edges; default 0.4 */
  thresholdLow?: number;
  /** 0-1, default 0.7 */
  thresholdHigh?: number;
  /** gaussian blur sigma in px; default 1.4 */
  blur?: number;
  /** invert: black edges on white. default true */
  invert?: boolean;
}

export function edgesPipeline(
...
function gaussianBlur(gray: Float32Array, w: number, h: number, sigma: number): Float32Array {
  ...
  for (let i = 0; i < kernel.length; i++) kernel[i] /= sum[0];
...
function nonMaxSuppress(mag: Float32Array, dir: Float32Array, w: number, h: number): Float32Array {
  const out = new Float32Array(w * h);
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      const idx = y * w + x;
...
function doubleThreshold(suppressed: Float32Array, w: number, h: number, tH: number, tL: number): Uint8Array {
  let max = 0;
  for (let i = 0; i < suppressed.length; i++) if (suppressed[i] > max) max = suppressed[i];
  if (max < 1) max = 1;
  ...
  for (let i = 0; i < suppressed.length; i++) {
    ...
    if (v >= highTh) out[i] = strong;
    else if (v >= lowTh) out[i] = weak;
    else out[i] = 0;
  }
    ...
    for (let x = 1; x < w - 1; x++) {
      ...
      for (let dy = -1; dy <= 1 && !promote; dy++) {
        for (let dx = -1; dx <= 1 && !promote; dx++) {
          ...
          if (out[(y + dy) * w + x + dx] === strong) promote = true;
        }
      }
      if (promote) out[idx] = strong;
      else out[idx] = 0;
    }
  }
  return out;
