...
| --- | --- | --- | --- | --- | --- |
| Product is one Workflow surfaced through Canvas and Table views, plus a global Agent drawer | Public README describes these three surfaces | `App.tsx` mounts `WorkflowWorkspace`, `TableWorkspace` and `AgentWorkspace`; Agent is hosted inside `StudioRightDrawer`, not as a peer route | Static route/mount inspection | `TRUE` at naming level | No |
...
| Agent is a spatial production workspace rather than only a Workflow drawer | Public copy presents an Agent surface | Agent is now hosted in the global right drawer (`StudioRightDrawer`); the standalone `AgentWorkspace` mount has been rolled back, and Workflow has a separate Production Crew status area | Source inspection | `FALSE` for the peer-workspace claim | Yes, if the public UX claim exceeds evidence |
