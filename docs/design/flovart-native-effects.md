...
| `tools/flovart/`、`agent/` | CLI、命令描述、当前 Browser 绑定 | 独立公开 MCP 已实现 |
...
5. **接 CLI/MCP 与 Agent**：基于现有命令描述派生 schema，MCP 使用官方 SDK。新效果命令只有实现、schema 和验证齐备后才进入 Skill；当前命令仍按现有 CLI 文档执行。
...
有效文档只有一个主设计、一份精简术语表、必要 ADR 和实际支持矩阵。实现任务进 todo，真正完成的变更才进 pending-test，用户验证后才更新正式功能。历史审计放证据区并标明其基线；过时方案从活动文档删除，由 Git 历史恢复，不再复制到新的“历史目标”目录。
