import { Archive, Bot, Circle, History, ImagePlus, Link2, MessageSquare, PenLine, Plus, ScrollText, Send, Trash2, X } from 'lucide-react';
...
export function WorkflowAgentPanel({ project, onClose, onProjectChange, onOnlineTurn, embedded = false, onActivityChange }: WorkflowAgentPanelProps) {
  ...
  return (
    <aside className={`workflow-agent ${embedded ? 'is-embedded' : ''}`}>
      <header className={embedded ? 'workflow-agent__utility' : undefined}>
        {!embedded && <><Bot size={16} /><strong>Workflow Agent</strong></>}
        <div className="workflow-agent__mode"><button type="button" className={mode === 'online' ? 'is-active' : ''} onClick={() => switchMode('online')}>网站</button><button type="button" className={mode === 'local' ? 'is-active' : ''} onClick={() => switchMode('local')}>本机</button></div>
        <span className={`workflow-agent__status is-${visibleStatus}`}><Circle size={8} />{statusLabel(visibleStatus)}</span>
        <button type="button" title="新对话" aria-label="新对话" onClick={() => void newConversation()}><Plus size={15} /></button>
        {!embedded && <button type="button" aria-label="关闭 Agent" onClick={onClose}><X size={15} /></button>}
      </header>
