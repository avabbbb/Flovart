  ...
  return (
    <button
      ...
      onClick={event => {
        ...
        const x = minX + Math.max(0, Math.min(150, event.clientX - rect.left - 8)) / scale;
        const y = minY + Math.max(0, Math.min(96, event.clientY - rect.top - 8)) / scale;
        onCenter(x, y);
      }}
    >
      {nodes.map(node => (
        <span
          ...
          style={{ left: (node.position.x - minX) * scale, top: (node.position.y - minY) * scale, width: Math.max(3, node.width * scale), height: Math.max(3, node.height * scale) }}
        />
      ))}
