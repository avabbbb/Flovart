...
- WSL with Agent inside the same distribution: run the same commands and connect the Windows browser to `http://localhost:17372`.
...
Install the public MCP wrapper with:

```bash
npx flovart-cli init --host codex
```

It exposes the same available Runtime, Research, and Workspace commands. The in-app Managed Agent receives only available `workflow.*` tools for the visible project.
