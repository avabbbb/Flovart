

const QUICK_PROMPTS_EN = [
    { name: 'Image to Figure', value: 'turn this photo into a character figure. Behind it, place a box with the character’s image printed on it, and a computer showing the Blender modeling process on its screen. In front of the box, add a round plastic base with the character figure standing on it. set the scene indoors if possible' },
    { name: 'Change Character Perspective', value: 'change the Camera anglo a high-angled selfie perspective looking down at the woman, while preserving her exact facial features, expression, and clothing, Maintain the same living room interior background with the sofa, natural lighting, and overall photographic composition and style.' },
    { name: 'Architecture to Model', value: 'convert this photo into a architecture model. Behind the model, there should be a cardboard box with an image of the architecture from the photo on it. There should also be a computer, with the content on the computer screen showing the Blender modeling process of the figurine. In front of the cardboard box, place a cardstock and put the architecture model from the photo I provided on it. I hope the PVC material can be clearly presented. It would be even better if the background is indoors.' },
    { name: 'Combine Objects', value: 'Combine them together' },
    { name: 'High-Res Restoration', value: 'Enhance this image to high resolution' },
    { name: 'Image to Line Art', value: 'Turn into a line art hand-drawn sketch' },
    { name: 'Color with Palette', value: 'Color accurately using the color palette' },
    { name: 'Generate Character Sheet', value: 'Generate a Character Design Sheet for me\n\nScale settings (comparison of different heights, head-to-body ratio, etc.)\n\nThree-view drawing (front, side, back)\n\nExpression Sheet\n\nPose Sheet → Various common poses\n\nCostume Design' },
    { name: 'Virtual/Reality Blend', value: 'Add a couple sitting at the seats, happily drinking coffee and talking, with characters in a cute, thick-lined illustration style' },
    { name: 'Anime to Photorealistic', value: 'Generate a highly detailed photo of a girl cosplaying this illustration, at Comiket. Exactly replicate the same pose, body posture, hand gestures, facial expression, and camera framing as in the original illustration. Keep the same angle, perspective, and composition, without any deviation' },
    { name: 'Pose Reference', value: 'Accurately change the person to the pose in the reference image, professional studio shot' },
    { name: 'Image to Action Figure', value: 'Transform the the person in the photo into an action figure, styled after [CHARACTER_NAME] from [SOURCE / CONTEXT]. \nNext to the figure, display the accessories including [ITEM_1], [ITEM_2], and [ITEM_3]. \nOn the top of the toy box, write "[BOX_LABEL_TOP]", and underneath it, "[BOX_LABEL_BOTTOM]". \nPlace the box in a [BACKGROUND_SETTING] environment. \nVisualize this in a highly realistic way with attention to fine details.' },
    { name: 'Image to Funko Pop', value: "Transform the person in the photo into the style of a Funko Pop figure packaging box, presented in an isometric perspective. Label the packaging with the title 'ZHOGUE'. Inside the box, showcase the figure based on the person in the photo, accompanied by their essential items (such as cosmetics, bags, or others). Next to the box, also display the actual figure itself outside of the packaging, rendered in a realistic and lifelike style." },
    { name: 'Image to LEGO', value: "Transform the person in the photo into the style of a LEGO minifigure packaging box, presented in an isometric perspective. Label the packaging with the title 'ZHOGUE'. Inside the box, showcase the LEGO minifigure based on the person in the photo, accompanied by their essential items (such as cosmetics, bags, or others) as LEGO accessories. Next to the box, also display the actual LEGO minifigure itself outside of the packaging, rendered in a realistic and lifelike style." },
    { name: 'Image to Knit Doll', value: 'A close-up, professionally composed photograph showing a handmade crocheted yarn doll being gently held in both hands. The doll has a rounded shape and an adorable chibi-style appearance, with vivid color contrasts and rich details. The hands holding the doll appear natural and tender, with clearly visible finger posture, and the skin texture and light-shadow transitions look soft and realistic, conveying a warm, tangible touch. The background is slightly blurred, depicting an indoor setting with a warm wooden tabletop and natural light streaming in through a window, creating a cozy and intimate atmosphere. The overall image conveys a sense of exquisite craftsmanship and a cherished, heartwarming emotion.' },
    { name: 'Image to Barbie Doll', value: "Transform the person in the photo into the style of a Barbie doll packaging box, presented in an isometric perspective. Label the packaging with the title 'ZHOGUE'. Inside the box, showcase the Barbie doll version of the person from the photo, accompanied by their essential items (such as cosmetics, bags, or others) designed as stylish Barbie accessories. Next to the box, also display the actual Barbie doll itself outside of the packaging, rendered in a realistic and lifelike style, resembling official Barbie promotional renders" },
    { name: 'Anything to Gundam', value: "Transform the person in the photo into the style of a Gundam model kit packaging box, presented in an isometric perspective. Label the packaging with the title 'ZHOGUE'. Inside the box, showcase a Gundam-style mecha version of the person from the photo, accompanied by their essential items (such as cosmetics, bags, or others) redesigned as futuristic mecha accessories. The packaging should resemble authentic Gunpla boxes, with technical illustrations, instruction-manual style details, and sci-fi typography. Next to the box, also display the actual Gundam-style mecha figure itself outside of the packaging, rendered in a realistic and lifelike style, similar to official Bandai promotional renders." },
    { name: 'Cyber-Baby Generator', value: 'Generate what the child of the two people in the picture would look like, professional photography' },
    { name: 'Product Design to Reality', value: 'turn this illustration of a perfume into a realistic version, Frosted glass bottle with a marble cap' },
    { name: 'Photo to Pro Shot', value: 'Transform the person in the photo into highly stylized ultra-realistic portrait, with sharp facial features and flawless fair skin, standing confidently against a bold green gradient background. Dramatic, cinematic lighting highlights her facial structure, evoking the look of a luxury fashion magazine cover. Editorial photography style, high-detail, 4K resolution, symmetrical composition, minimalistic background' },
    { name: 'Lighting Reference', value: 'Change the lighting of the original image to match the reference image, professional photography' },
    { name: 'Generate Painting Process', value: 'Generate a four-panel painting process for the character. Step 1: Line art, Step 2: Flat colors, Step 3: Add shadows, Step 4: Final details. No text.' },
    { name: 'Any Style to Realistic', value: 'turn this illustration into realistic version' },
    { name: 'Image to Keychain', value: 'Turn this photo into a cute charm hanging on the bag in the photo\nTurn this photo into a flat acrylic keychain hanging on the bag in the photo\nTurn this photo into a flat rubber keychain hanging on the bag in the photo' },
    { name: 'Superimpose Effects', value: 'Superimpose the effect from the effect image onto the main photo' },
    { name: 'Product Packaging Mockup', value: 'Place the image on a packaging box and set it in a minimalist scene, professional photography' },
    { name: 'Virtual Makeup Try-on', value: 'Apply the makeup from the image to the person, while maintaining their original pose' },
    { name: 'Expression Reference', value: 'Change the person\'s expression to match the new image' },
];

const QUICK_PROMPTS_ZHO = [
    { name: '图片变手办', value: '将这张照片变成一个人物手办。在它后面，放置一个印有角色形象的盒子，以及一台屏幕上显示着Blender建模过程的电脑。在盒子前面，加一个圆形的塑料底座，上面站着角色手办。如果可能的话，将场景设置在室内' },
    { name: '改变人物视角', value: '将相机角度改为高角度自拍视角，俯视女性，同时保留她确切的面部特征、表情和服装，保持相同的客厅室内背景、沙发、自然光以及整体的摄影构图和风格。' },
    { name: '建筑变模型', value: '将这张照片转换成一个建筑模型。模型后面应该有一个纸板箱，上面印有照片中建筑的图片。还应该有一台电脑，电脑屏幕上显示着该模型的Blender建模过程。在纸板箱前面，放置一张卡纸，并把照片中的建筑模型放在上面。我希望PVC材质能清晰呈现。如果背景是室内的就更好了。' },
    { name: '组合对象', value: '把它们组合起来' },
    { name: '高清修复', value: '将此图片增强为高分辨率' },
    { name: '图片转线稿', value: '变成线稿手绘图' },
    { name: '使用调色板上色', value: '准确使用色卡上色' },
    { name: '生成角色设定', value: '为我生成人物的角色设定（Character Design）\n\n比例设定（不同身高对比、头身比等）\n\n三视图（正面、侧面、背面）\n\n表情设定（Expression Sheet） \n\n动作设定（Pose Sheet） → 各种常见姿势\n\n服装设定（Costume Design）' },
    { name: '虚拟与现实融合', value: '在图中加上一对情侣坐在座位上开心的喝咖啡和交谈，人物都是粗线稿可爱插画风格' },
    { name: '动漫转照片', value: '生成一张在Comiket上cosplay这张插画的女孩的高度详细照片。精确复制与原始插画相同的姿势、身体姿态、手势、面部表情和相机取景。保持相同的角度、透视和构图，没有任何偏差' },
    { name: '姿势参考', value: '人物准确换成姿势图的姿势，专业摄影棚拍摄' },
    { name: '图片转可动人偶', value: '将照片中的人物转换成一个可动人偶，风格模仿[来源/背景]中的[角色名称]。\n在人偶旁边，展示包括[物品1]、[物品2]和[物品3]在内的配件。\n在玩具盒顶部写上“[顶部标签]”，在其下方写上“[底部标签]”。\n将盒子放置在[背景设定]环境中。\n以高度逼真的方式并注重细节来呈现此场景。' },
    { name: '图片转Funko Pop', value: '将照片中的人物转换成Funko Pop人偶包装盒的风格，以等距视角呈现。包装上标注标题“ZHOGUE”。盒子内展示基于照片中人物的人偶，并附带他们的基本物品（如化妆品、包包或其他）。在盒子旁边，也以逼真生动的风格展示包装外的实际人偶。' },
    { name: '图片转乐高', value: '将照片中的人物转换成乐高小人仔包装盒的风格，以等距视角呈现。包装上标注标题“ZHOGUE”。盒子内展示基于照片中人物的乐高小人仔，并附带他们的基本物品（如化妆品、包包或其他）作为乐高配件。在盒子旁边，也以逼真生动的风格展示包装外的实际乐高小人仔。' },
    { name: '图片转针织娃娃', value: '一张特写、专业构图的照片，展示了一个手工钩编的毛线娃娃被温柔地捧在双手之中。娃娃造型圆润，呈可爱的赤壁风格，色彩对比鲜明，细节丰富。捧着娃娃的双手显得自然而温柔，手指姿势清晰可见，皮肤纹理和光影过渡看起来柔软逼真，传达出温暖、可触的感觉。背景略微模糊，描绘了一个室内场景，有温暖的木质桌面和透过窗户射入的自然光，营造出舒适亲密的氛围。整体画面传达出一种精致的手工艺感和被珍视、温馨的情感。' },
    { name: '图片转芭比娃娃', value: '将照片中的人物转换成芭比娃娃包装盒的风格，以等距视角呈现。包装上标注标题“ZHOGUE”。盒子内展示照片中人物的芭比娃娃版本，并附带他们的基本物品（如化妆品、包包或其他）设计成时尚的芭比配件。在盒子旁边，也以逼真生动的风格展示包装外的实际芭比娃娃，类似于官方芭比的宣传渲染图。' },
    { name: '万物皆可高达', value: '将照片中的人物转换成高达模型套件包装盒的风格，以等距视角呈现。包装上标注标题“ZHOGUE”。盒子内展示照片中人物的高达风格机甲版本，并附带他们的基本物品（如化妆品、包包或其他）重新设计成未来派的机甲配件。包装应类似于正品高达模型盒子，带有技术插图、说明书风格的细节和科幻字体。在盒子旁边，也以逼真生动的风格展示包装外的实际高达风格机甲，类似于官方万代的宣传渲染图。' },
    { name: '赛博子女生成器', value: '生成图中两人物所生孩子的样子，专业摄影' },
    { name: '产品设计转实物', value: '将这个香水插图变成逼真版本，磨砂玻璃瓶配大理石盖' },
    { name: '照片转专业摄影', value: '将照片中的人物转换成高度风格化的超逼真肖像，面部特征鲜明，皮肤白皙无瑕，自信地站在大胆的绿色渐变背景前。戏剧性的电影灯光突显了她的面部结构，唤起了奢侈时尚杂志封面的感觉。编辑摄影风格，高细节，4K分辨率，对称构图，简约背景' },
    { name: '灯光参考', value: '原图换成参考图打光，专业摄影' },
    { name: '生成绘画过程', value: '为人物生成绘画过程四宫格，第一步：线稿，第二步平铺颜色，第三步：增加阴影，第四步：细化成型。不要文字' },
    { name: '任意风格转写实', value: '将这个插图变成逼真版本' },
    { name: '图片转钥匙扣', value: '把这张照片变成一个可爱挂件 挂在 照片的包包上\n把这张照片变成一个亚克力材质的扁平钥匙扣 挂在 照片的包包上\n把这张照片变成一个橡胶材质的扁平钥匙扣 挂在照片的包包上' },
    { name: '叠加效果', value: '为图片照片叠加上效果图片的效果' },
    { name: '产品包装样机', value: '把图片贴在包装盒上，并放在极简设计的布景中，专业摄影' },
    { name: '虚拟试妆', value: '为人物化上图片的妆，还保持人物原本的姿势' },
    { name: '表情参考', value: '人物换成新图片的表情' },
];

export const translations = {
  en: {
    toolbar: {
      undo: 'Undo',
      redo: 'Redo',
      select: 'Select & Move',
      pan: 'Pan Canvas',
      shapes: 'Shapes',
      drawingTools: 'Drawing Tools',
      draw: 'Pencil',
      highlighter: 'Inpaint Brush',
      lasso: 'Lasso Select',
      rectangle: 'Rectangle',
      circle: 'Circle',
      triangle: 'Triangle',
      arrow: 'Arrow',
      line: 'Line',
      text: 'Text',
      erase: 'Erase',
      strokeColor: 'Stroke Color',
      strokeWidth: 'Stroke Width',
      upload: 'Upload Image',
      settings: 'Canvas Settings',
      layers: 'Layers',
      crop: {
        title: 'Crop Image',
        cancel: 'Cancel',
        confirm: 'Confirm',
      },
    },
    inlinePrompt: {
        imageTitle: '🎨 IMG_MATRIX',
        videoTitle: '🎬 FILM_MATRIX',
        imagePlaceholder: 'Enter prompt or type @ to quote whiteboard asset...',
        videoPlaceholder: 'Describe camera motion vector or type @ to bind first frame...',
        statusIdle: 'STANDBY',
        statusQueued: 'QUEUED',
        statusRunning: 'COMPUTING',
        statusSuccess: 'READY',
        statusError: 'ERROR',
        statusReady: 'CORE READY',
        slotFirstFrame: 'POSTER',
        slotStyleRef: 'STYLE',
        slotControlNet: 'CONTROL',
        slotContext: 'CONTEXT',
        ignite: 'IGNITE',
        computing: 'COMPUTING',
        queue: 'QUEUE',
        model: 'MODEL',
        uploadReference: 'Upload reference',
        removeReference: 'Remove reference',
        noProvider: 'Provider key missing',
    },
    promptBar: {
        placeholderDefault: 'Describe an image to generate from scratch...',
        placeholderDefaultVideo: 'Describe a video to generate from scratch...',
        placeholderSingle: 'Describe the changes or select a quick effect...',
        placeholderMultiple: (count: number) => `Using ${count} elements as context. Describe the result or select an effect...`,
        generate: 'Generate',
        imageMode: 'Image',
        videoMode: 'Video',
        aspectRatioHorizontal: 'Horizontal (16:9)',
        aspectRatioVertical: 'Vertical (9:16)',
    },
    quickPromptsAriaLabel: 'Select a quick effect',
    quickPrompts: QUICK_PROMPTS_EN,
    myEffects: {
        title: 'My Effects',
        saveEffectTooltip: 'Save as custom effect',
        saveEffectPrompt: 'Enter a name for this effect:',
        deleteEffectTooltip: 'Delete effect',
        noEffects: 'No custom effects yet.',
        defaultName: 'My Custom Effect',
    },
    settings: {
        title: 'Settings',
        language: 'Language',
        uiTheme: 'UI Theme',
        color: 'Color',
        opacity: 'Opacity',
        actionButtonsTheme: 'Action Buttons',
        canvas: 'Canvas',
        backgroundColor: 'Background Color',
        mouseWheel: 'Mouse Wheel',
        zoom: 'Zoom',
        scroll: 'Scroll',
    },
    contextMenu: {
        copy: 'Copy',
        download: 'Download',
        crop: 'Crop',
        delete: 'Delete',
        fillColor: 'Fill Color',
        fontColor: 'Font Color',
        fontSize: 'Font Size',
        strokeColor: 'Stroke Color',
        strokeWidth: 'Stroke Width',
        strokeStyle: 'Stroke Style',
        solid: 'Solid',
        dashed: 'Dashed',
        dotted: 'Dotted',
        borderRadius: 'Corner Radius',
        bringForward: 'Bring Forward',
        sendBackward: 'Send Backward',
        bringToFront: 'Bring to Front',
        sendToBack: 'Send to Back',
        rasterize: 'Rasterize Selection',
        group: 'Group',
        ungroup: 'Ungroup',
        alignment: {
            alignLeft: 'Align Left',
            alignCenter: 'Align Center',
            alignRight: 'Align Right',
            alignTop: 'Align Top',
            alignMiddle: 'Align Middle',
            alignBottom: 'Align Bottom',
        },
    },
    browse: {
        title: 'Start by browsing',
        subtitle: 'Peek at templates and recent sessions before creating anything.',
        recent: 'Recent',
        templates: 'Templates',
        emptyRecent: 'No recent sessions yet.',
        useTemplate: 'Use this template',
        offlineMode: 'Offline browsing',
        offlineHint: 'You can explore templates and drafts offline; add an API key when you are ready to generate.',
    },
    agentPanel: {
        title: 'Chat',
        statusConnecting: 'Connecting',
        statusError: 'Connection failed',
        statusReady: 'Ready',
        statusNeedConfig: 'Setup needed',
        statusWorkspaceDisconnected: 'Workspace disconnected',
        statusSyncing: 'Syncing workspace',
        openModelMapping: 'Open model mapping',
        textRouteRequired: 'Configure an available route for the Agent text capability in Settings → Model mapping.',
        setupCredential: 'The assistant needs an AI service credential before it can chat. Add an API key or finish model mapping to continue.',
        setupOffline: 'Flovart needs a running collaborator such as Codex, Claude, or OpenCode. Connect one on the Agent page, or add an AI service in Settings to use the built-in assistant.',
        toolWaitingApproval: 'Waiting for your Production approval',
        toolPreparingPlan: 'Reading or preparing the Production plan',
        toolUpdatingDraft: 'Updating the current Workflow draft',
        addApiKey: 'Add API key',
        tryOffline: 'Try offline mode',
        openHosts: 'Pick a collaborator',
        history: 'Chat history',
        newChat: 'New chat',
        newChatHint: 'Start a fresh conversation',
        noHistory: 'No past conversations',
        composerPlaceholder: 'Tell Flovart Agent what you want to make',
        composerNeedConfig: 'Add an API key to chat with the agent',
        composerOffline: 'Agent unavailable — browse templates or add a key',
        send: 'Send',
        stop: 'Stop',
        addAttachment: 'Add attachment',
        referenceNodes: 'Reference a workflow node',
        referenceNodesHint: 'Pick from the current canvas',
        referenceAssets: 'Add from My Assets',
        referenceAssetCount: (count: number) => `${count} local assets`,
        generationMode: 'Generation mode',
        productionContext: 'Production context',
        composerAria: 'Describe your creation, or @ to reference a workflow/node/asset',
        safetyBoundary: 'Safety boundary',
        confirmPolicy: 'Confirmation policy',
        manualMode: 'Manual mode',
        autoMode: 'Auto mode',
        manualModeHint: 'Ask before every write',
        autoModeHint: 'Reversible ops run automatically; paid/destructive still ask',
        resync: 'Re-sync workspace',
        nodes: 'Nodes',
        connections: 'Connections',
        changes: 'Changes',
        confirmTitle: 'Agent asks for confirmation',
        deny: 'Deny',
        allow: 'Allow',
    },
    promptBarExtra: {
        addService: 'Add AI service',
        configureService: 'Configure AI service',
        addServiceHint: 'Add an AI service to start generating',
        pickModelFirst: 'Pick a product model first',
        mapRouteFirst: 'Map a route for this capability first',
        generateFailed: 'Generation failed',
        emptyPromptHint: 'Describe what you want to generate or change',
        generatingHint: 'Generating — keep this workflow open',
        readyHint: 'Ready — Ctrl+Enter to generate',
        selectedElements: (count: number) => `${count} elements selected`,
        describeChangeHint: 'Describe “how to change” rather than “what it is”',
        attachedRefs: (count: number) => `${count} references added`,
        mentionNodeHint: 'Type @ to reference workflow nodes',
        dropMediaHint: 'Drop image/video/audio references',
        placeholderDefault: 'Use @ to reference images in the workflow, e.g. swap the person in @image1 for the rabbit in @image2',
        placeholderSingle: 'Describe what to do with this element',
        placeholderMultiple: (count: number) => `${count} elements selected — describe the combined result`,
        noServiceChip: 'No AI service configured',
        noServiceTitle: 'No access credential yet — click to open Settings',
        openModelMapping: 'Open model mapping',
        agentRouteMissing: 'No agent text route configured yet',
        pickModel: 'Choose a model',
        modelNotMapped: (name: string) => `${name} has no API route mapped`,
        goConfigure: 'Configure →',
        retry: 'Retry',
        startGenerating: 'Generate',
        generateVersions: (count: number) => `Generate ${count} versions`,
        stopGeneration: 'Stop generating',
        stopSeedance: 'Stop and try to cancel the task',
        stopSeedanceTitle: 'Stops the local wait and tries to cancel upstream; once upstream is generating it may still bill',
    },
    settingsExtra: {
        title: 'Settings',
        close: 'Close settings',
        categoryAria: 'Settings category',
        apiTab: 'AI Services',
        apiTabDetail: 'Providers, API keys and connection status',
        modelsTab: 'Model Mapping',
        modelsTabDetail: 'Product models and route priority',
        securityTab: 'Security',
        securityTabDetail: 'Local data and exit policy',
        addService: '+ Add AI service',
        emptyServicesTitle: 'No AI service configured yet',
        emptyServicesHint: 'Click “+ Add AI service” at the top right to add a direct access credential',
        mappingTitle: 'Model Mapping',
        mappingIntro: 'Pick a Flovart product model or text capability, then bind an AI service route. This is the only routing source.',
        addMappingFirst: 'Add a direct access credential under “AI Services” first, then build model mappings.',
    },
  },
  zho: {
    toolbar: {
      undo: '撤销',
      redo: '重做',
      select: '选择并移动',
      pan: '平移画布',
      shapes: '形状',
      drawingTools: '绘图工具',
      draw: '铅笔',
      highlighter: '局部重绘',
      lasso: '套索选择',
      rectangle: '矩形',
      circle: '圆形',
      triangle: '三角形',
      arrow: '箭头',
      line: '直线',
      text: '文本',
      erase: '橡皮擦',
      strokeColor: '描边颜色',
      strokeWidth: '描边宽度',
      upload: '上传图片',
      settings: '画布设置',
      layers: '图层',
      crop: {
        title: '裁剪图片',
        cancel: '取消',
        confirm: '确认',
      },
    },
    inlinePrompt: {
        imageTitle: '🎨 图像生成矩阵',
        videoTitle: '🎬 电影运镜控制',
        imagePlaceholder: '输入生图提示词，或打入 @ 引用画布图层...',
        videoPlaceholder: '输入镜头运动矢量，或打入 @ 绑定首帧垫图...',
        statusIdle: '待命',
        statusQueued: '排队中',
        statusRunning: '异步渲染中',
        statusSuccess: '就绪',
        statusError: '异常',
        statusReady: '计算核心就绪',
        slotFirstFrame: '首帧',
        slotStyleRef: '风格',
        slotControlNet: '控网',
        slotContext: '参考',
        ignite: '核爆点火',
        computing: '异步渲染中',
        queue: '队列',
        model: '模型',
        uploadReference: '上传参考图',
        removeReference: '移除参考',
        noProvider: '未配置生成服务密钥',
    },
    promptBar: {
        placeholderDefault: '描述一下您想生成什么样的图片...',
        placeholderDefaultVideo: '描述一下您想生成什么样的视频...',
        placeholderSingle: '描述要做的修改，或选择一个快捷效果...',
        placeholderMultiple: (count: number) => `已选择 ${count} 个元素作为上下文。请描述结果或选择一个效果...`,
        generate: '生成',
        imageMode: '图片',
        videoMode: '视频',
        aspectRatioHorizontal: '横向 (16:9)',
        aspectRatioVertical: '纵向 (9:16)',
    },
    quickPromptsAriaLabel: '选择一个快捷效果',
    quickPrompts: QUICK_PROMPTS_ZHO,
    myEffects: {
        title: '我的效果',
        saveEffectTooltip: '保存为自定义效果',
        saveEffectPrompt: '为此效果输入一个名称：',
        deleteEffectTooltip: '删除效果',
        noEffects: '暂无自定义效果。',
        defaultName: '我的自定义效果',
    },
    settings: {
        title: '设置',
        language: '语言',
        uiTheme: '界面主题',
        color: '颜色',
        opacity: '不透明度',
        actionButtonsTheme: '功能按钮',
        canvas: '画布',
        backgroundColor: '背景颜色',
        mouseWheel: '鼠标滚轮',
        zoom: '缩放',
        scroll: '滚动',
    },
    contextMenu: {
        copy: '复制',
        download: '下载',
        crop: '裁剪',
        delete: '删除',
        fillColor: '填充颜色',
        fontColor: '字体颜色',
        fontSize: '字号',
        strokeColor: '描边颜色',
        strokeWidth: '描边宽度',
        strokeStyle: '线条样式',
        solid: '实线',
        dashed: '虚线',
        dotted: '点线',
        borderRadius: '圆角大小',
        bringForward: '上移一层',
        sendBackward: '下移一层',
        bringToFront: '置于顶层',
        sendToBack: '置于底层',
        rasterize: '栅格化选中项',
        group: '创建编组',
        ungroup: '取消编组',
        alignment: {
            alignLeft: '左对齐',
            alignCenter: '水平居中',
            alignRight: '右对齐',
            alignTop: '顶对齐',
            alignMiddle: '垂直居中',
            alignBottom: '底对齐',
        },
    },
    browse: {
        title: '先逛逛',
        subtitle: '在动手创作之前，先看看模板和最近会话。',
        recent: '最近',
        templates: '模板',
        emptyRecent: '暂无最近会话。',
        useTemplate: '用这个模板',
        offlineMode: '离线浏览中',
        offlineHint: '离线也能浏览模板和草稿；想生成时再添加 API Key。',
    },
    agentPanel: {
        title: '对话',
        statusConnecting: '连接中',
        statusError: '连接失败',
        statusReady: '已就绪',
        statusNeedConfig: '需要配置',
        statusWorkspaceDisconnected: '工作区断开',
        statusSyncing: '同步工作区',
        openModelMapping: '打开模型映射',
        textRouteRequired: '请在设置的“模型映射”中为 Agent 文本能力配置可用线路。',
        setupCredential: '助手需要一个可用的 AI 服务访问凭证才能对话。添加一个 API Key 或完成模型映射后即可开始。',
        setupOffline: 'Flovart 需要一个正在运行的协作 Agent（如 Codex、Claude 或 OpenCode）才能继续。打开“协作”页签连接一个助手，或在设置中添加 AI 服务后使用内置助手。',
        toolWaitingApproval: '等待你的 Production 授权',
        toolPreparingPlan: '正在读取或编译 Production Plan',
        toolUpdatingDraft: '正在操作同一 Workflow Draft',
        addApiKey: '添加 API Key',
        tryOffline: '试试离线模式',
        openHosts: '选择协作助手',
        history: '历史对话',
        newChat: '新对话',
        newChatHint: '开启一段全新会话',
        noHistory: '暂无历史对话',
        composerPlaceholder: '告诉 Flovart Agent 你想制作什么',
        composerNeedConfig: '请先配置 Agent 文本模型映射',
        composerOffline: '助手暂不可用——可以先浏览模板，或添加 API Key',
        send: '发送',
        stop: '停止',
        addAttachment: '添加附件',
        referenceNodes: '引用工作流节点',
        referenceNodesHint: '从当前画布选择',
        referenceAssets: '从我的素材添加',
        referenceAssetCount: (count: number) => `${count} 个本地资产`,
        generationMode: '生成模式',
        productionContext: '制作上下文',
        composerAria: '开始你的创作，或者 @ 引用工作流/节点/资源',
        safetyBoundary: '安全边界',
        confirmPolicy: '确认策略',
        manualMode: '手动模式',
        autoMode: '自动模式',
        manualModeHint: '每个写操作前询问',
        autoModeHint: '可逆操作自动推进；付费、删除仍确认',
        resync: '重新同步',
        nodes: '节点',
        connections: '连接',
        changes: '变更',
        confirmTitle: 'Agent 请求确认',
        deny: '拒绝',
        allow: '允许',
    },
    promptBarExtra: {
        addService: '添加 AI 服务',
        configureService: '配置 AI 服务',
        addServiceHint: '添加 AI 服务以开始生成',
        pickModelFirst: '请先明确选择产品模型',
        mapRouteFirst: '请先配置当前能力的模型映射',
        generateFailed: '生成失败',
        emptyPromptHint: '输入你想生成或修改的画面',
        generatingHint: '正在生成，请保持工作流打开',
        readyHint: '准备就绪，Ctrl+Enter 生成',
        selectedElements: (count: number) => `已选中 ${count} 个元素`,
        describeChangeHint: '描述“怎么改”比描述“是什么”更有效',
        attachedRefs: (count: number) => `已添加 ${count} 个参考`,
        mentionNodeHint: '输入 @ 可引用工作流节点',
        dropMediaHint: '支持拖入图片/视频/音频参考',
        placeholderDefault: '使用 @ 引用工作流中的图片，例如：把 @图片1 的人物替换为 @图片2 的兔子',
        placeholderSingle: '描述你想对当前元素做什么',
        placeholderMultiple: (count: number) => `已选中 ${count} 个元素，补充组合生成描述`,
        noServiceChip: '未配置 AI 服务',
        noServiceTitle: '尚未配置访问凭证，点击打开设置',
        openModelMapping: '打开模型映射',
        agentRouteMissing: '尚未配置 Agent 文本线路',
        pickModel: '选择模型',
        modelNotMapped: (name: string) => `${name} 尚未映射 API 线路`,
        goConfigure: '去配置 →',
        retry: '重试',
        startGenerating: '开始生成',
        generateVersions: (count: number) => `生成 ${count} 版`,
        stopGeneration: '停止生成',
        stopSeedance: '停止并尝试取消任务',
        stopSeedanceTitle: '停止本地等待并尝试取消上游任务；若已进入生成阶段，上游仍可能继续计费',
    },
    settingsExtra: {
        title: '设置',
        close: '关闭设置',
        categoryAria: '设置分类',
        apiTab: 'AI 服务',
        apiTabDetail: 'Provider、API Key 和连接状态',
        modelsTab: '模型映射',
        modelsTabDetail: '产品模型与路线优先级',
        securityTab: '安全',
        securityTabDetail: '本地数据和退出策略',
        addService: '+ 添加 AI 服务',
        emptyServicesTitle: '还没有配置 AI 服务',
        emptyServicesHint: '点击右上方「+ 添加 AI 服务」按钮添加网页直连访问凭证',
        mappingTitle: '模型映射',
        mappingIntro: '先选择 Flovart 的产品模型或文本能力，再绑定 AI 服务线路。这里是唯一选路来源。',
        addMappingFirst: '请先在“AI 服务”中添加网页直连访问凭证，随后再建立模型映射。',
    },
  }
};
